...
interface Window {
  id: string;
  title: string;
  type: 'terminal' | 'files' | 'monitor' | 'about' | 'appstore';
  x: number;
  y: number;
  width: number;
  height: number;
...
function LinuxDesktop() {
  ...
  const openWindow = (type: Window['type']) => {
    const newWindow: Window = {
      ...
      title: type === 'terminal' ? 'Terminal' : type === 'files' ? 'Files' : type === 'monitor' ? 'System Monitor' : type === 'appstore' ? 'App Store' : 'About',
      type,
      x: 100 + windows.length * 30,
      y: 80 + windows.length * 30,
      width: type === 'terminal' ? 700 : type === 'monitor' ? 500 : type === 'appstore' ? 800 : 600,
      height: type === 'terminal' ? 450 : type === 'monitor' ? 400 : type === 'appstore' ? 600 : 500,
      zIndex: nextZIndex,
      minimized: false,
      maximized: false,
    };
    ...
    <div className="relative h-full w-full bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 overflow-hidden">
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-end gap-2 bg-black/40 backdrop-blur-xl px-4 py-2 rounded-2xl border border-white/20 z-[9998]">
        ...
        {windows.filter(w => w.minimized).map(w => (
          <DockIcon
            key={w.id}
            icon={w.type === 'terminal' ? '💻' : w.type === 'files' ? '📁' : w.type === 'monitor' ? '📊' : w.type === 'appstore' ? '🛍️' : 'ℹ️'}
            label={w.title}
